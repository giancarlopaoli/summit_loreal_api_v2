...
class AuthController extends Controller
{
    public function login(Request $request) {
        ...
        if (Auth::attempt(array_merge($credentials, ['status' => UserStatus::Activo]))) {
            ...
            $assigned_client = ($user->assigned_client->count() == 0) ? null : $user->assigned_client[0]->only(['id','name','last_name','mothers_name','document_type_id', 'document_number', 'customer_type', "created_at", 'document_type:id,name', 'email']);
