...
class DashboardController extends Controller
{
    ...
    public function dashboard_indicators(Request $request) {
    ...
    public function dashboard_monthly(Request $request) {
        ...
        $monthly_indicators = Operation::join("clients","clients.id","=","operations.client_id")
            ...
            ->selectRaw("coalesce((select sum(ov.amount) from operations ov where ov.client_id = 366 and year(ov.deposit_date) = year(operations.deposit_date) and month(ov.deposit_date) = month(operations.deposit_date) and ov.operation_status_id = 7 ),0) as volume_coril")
            ->selectRaw("coalesce((select sum(ov.amount) from operations ov where ov.client_id = 2815 and year(ov.deposit_date) = year(operations.deposit_date) and month(ov.deposit_date) = month(operations.deposit_date) and ov.operation_status_id = 7 ),0) as volume_mibanco")
            ->selectRaw("coalesce((select sum(ov.amount) from operations ov where ov.client_id = 3166 and year(ov.deposit_date) = year(operations.deposit_date) and month(ov.deposit_date) = month(operations.deposit_date) and ov.operation_status_id = 7 ),0) as volume_renta4")
            ->selectRaw("coalesce((select sum(ov.amount) from operations ov where ov.client_id = 4280 and year(ov.deposit_date) = year(operations.deposit_date) and month(ov.deposit_date) = month(operations.deposit_date) and ov.operation_status_id = 7 ),0) as volume_ripley")
            //->selectRaw("coalesce((select sum(ov.amount) from operations ov where ov.client_id = 4540 and year(ov.deposit_date) = year(operations.deposit_date) and month(ov.deposit_date) = month(operations.deposit_date) and ov.operation_status_id = 7 ),0) as volume_cajatru")
            ->selectRaw("coalesce((select sum(ov.amount) from operations ov where ov.client_id = 4889 and year(ov.deposit_date) = year(operations.deposit_date) and month(ov.deposit_date) = month(operations.deposit_date) and ov.operation_status_id = 7 ),0) as volume_bcp")
            ->selectRaw("coalesce((select sum(ov.amount) from operations ov where ov.client_id = 4960 and year(ov.deposit_date) = year(operations.deposit_date) and month(ov.deposit_date) = month(operations.deposit_date) and ov.operation_status_id = 7 ),0) as volume_cajaarequipa")
            ->selectRaw("coalesce((select sum(ov.amount) from operations ov where ov.client_id = 5091 and year(ov.deposit_date) = year(operations.deposit_date) and month(ov.deposit_date) = month(operations.deposit_date) and ov.operation_status_id = 7 ),0) as volume_kallpa")
            ->selectRaw("coalesce((select sum(ov.amount) from operations ov where ov.client_id = 5381 and year(ov.deposit_date) = year(operations.deposit_date) and month(ov.deposit_date) = month(operations.deposit_date) and ov.operation_status_id = 7 ),0) as volume_falabella")
            ->selectRaw("coalesce((select sum(ov.amount) from operations ov where ov.client_id = 5494 and year(ov.deposit_date) = year(operations.deposit_date) and month(ov.deposit_date) = month(operations.deposit_date) and ov.operation_status_id = 7 ),0) as volume_compartamos")
