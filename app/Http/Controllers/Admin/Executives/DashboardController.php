...
class DashboardController extends Controller
{
    ...
    public function dashboard(Request $request) {
        ...
        $grafico_ventas_diarias = DB::table('operations_view')
            ...
            ->selectRaw("(select sum(op.amount) from operations op inner join clients cl on op.client_id = cl.id where month(op.operation_date) = month(now()) and year(op.operation_date) = year(now()) and day(op.operation_date) = day(operations_view.operation_date) and op.operation_status_id in (2,3,4,5) and cl.executive_id = $executive_id) as volumen_pendiente")

            ->selectRaw("(select sum(op.comission_amount) from operations op inner join clients cl on op.client_id = cl.id where month(op.operation_date) = month(now()) and year(op.operation_date) = year(now()) and day(op.operation_date) = day(operations_view.operation_date) and op.operation_status_id in (2,3,4,5) and cl.executive_id = $executive_id) as comision_pendiente")

            /*->selectRaw("(select round(sum(op.comission_amount*(if($executive_id=2801,(select ga.comission_achieved from goals_achievement ga where ga.operation_executive_id = $executive_id and ga.operation_month = month(now()) and ga.operation_year = year(now())),0.1))),2) from operations op inner join clients cl on op.client_id = cl.id where month(op.operation_date) = month(now()) and year(op.operation_date) = year(now()) and day(op.operation_date) = day(operations_view.operation_date) and op.operation_status_id in (2,3,4,5) and cl.executive_id = $executive_id) as comision_ejecutivo_pendiente")*/
        ...
        return response()->json([
            ...
            'data' => [ 
                ...
                'ventas_diarias' => [
                    ...
                    'volumen_pendiente' => $grafico_ventas_diarias->pluck('volumen_pendiente'),
