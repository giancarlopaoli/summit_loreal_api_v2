...
class DashboardController extends Controller
{
    ...
    public function dashboard(Request $request) {
        ...
        $grafico_ventas_diarias = DB::table('operations_view')
            ...
            ->selectRaw("(select sum(op.amount) from operations op inner join clients cl on op.client_id = cl.id where month(op.deposit_date) = month(now()) and year(op.deposit_date) = year(now()) and day(op.deposit_date) = day(operations_view.operation_date) and op.operation_status_id in (2,3,4,5) and cl.executive_id = $executive_id) as volumen_pendiente")

            ->selectRaw("(select sum(op.comission_amount) from operations op inner join clients cl on op.client_id = cl.id where month(op.deposit_date) = month(now()) and year(op.deposit_date) = year(now()) and day(op.deposit_date) = day(operations_view.operation_date) and op.operation_status_id in (2,3,4,5) and cl.executive_id = $executive_id) as comision_pendiente")
